...
import type { User, LeaderUser } from '@client/src/components/business-ui/types/user';

/** 把 SDK 的 LeaderUser（驼峰）转为仓内 snake_case 结构 */
function toLeaderUser(leader?: SdkLeaderUser): LeaderUser | undefined {
  if (!leader) {
    return undefined;
  }
  return {
    employee_id: leader.employeeID || undefined,
    miaoda_user_id: leader.miaodaUserID || undefined,
  };
}

/** 把 UserInfo 的 full 字段映射为仓内 User 的 snake_case full 字段 */
function pickFullFields(userInfo: UserInfo): Partial<User> {
  return {
    nickname: userInfo.nickname,
    mobile: userInfo.mobile,
    gender: userInfo.gender,
    country: userInfo.country,
    work_station: userInfo.workStation,
    employee_no: userInfo.employeeNo,
    city: userInfo.city,
    job_title: userInfo.jobTitle,
    employee_type: userInfo.employeeType,
    leader: toLeaderUser(userInfo.leader),
    dotted_line_leaders: userInfo.dottedLineLeaders?.map(
      (l) => toLeaderUser(l)!,
    ),
  };
}
...
export function userInfoToUser(
  ...
  const rawUser: User = {
    ...
    employee_id: userInfo.employeeID || undefined,
    miaoda_user_id: userInfo.miaodaUserID || undefined,
    lark_id: userInfo.larkID || undefined,
    name: userInfo.name,
    avatar: userInfo.avatar?.image?.large,
    user_type: userInfo.userType,
    department: userInfo.department as any,
    ...pickFullFields(userInfo),
  };
